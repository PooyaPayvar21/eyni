...
export function TopDoctorsGrid({ doctors, cities }: { doctors: Doctor[]; cities: City[] }) {
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between mb-4">
        <label className="w-full sm:w-auto text-sm text-muted-foreground">
          ...
          <select
            ...
            className="mt-1 w-full sm:w-56 rounded-lg border bg-background px-3 py-2 text-sm"
      ...
      <div ref={containerRef} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
        {filtered.map((doc) => (
          <Link
            key={doc.id}
            href={`/doctors/${doc.slug}`}
            className="group rounded-2xl p-[1px] bg-gradient-to-br from-cyan-500/30 to-sky-500/30 hover:from-cyan-500/50 hover:to-sky-500/50 transition will-change-transform"
          >
            <div
              data-animate
